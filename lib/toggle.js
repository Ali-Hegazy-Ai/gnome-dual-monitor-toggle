...
// Robustly unpack a GLib.Variant boolean, handling nested variants.
// Mutter sometimes wraps booleans in extra 'v' variant layers, and
// deepUnpack() alone can silently fail across Mutter versions (e.g. GNOME 50).
function _unpackBool(variant) {
    if (variant == null) return false;
    try {
        let v = variant;
        while (typeof v?.get_type_string === 'function' && v.get_type_string() === 'v') {
            v = v.get_variant();
        }
        if (typeof v?.get_boolean === 'function') return v.get_boolean();
...
export const SecondMonitorToggle = GObject.registerClass(
    class SecondMonitorToggle extends QuickMenuToggle {
        ...
        _resetTimeout(timeoutProp, delayMs, callback) {
            ...
            this[timeoutProp] = GLib.timeout_add_once(GLib.PRIORITY_DEFAULT, delayMs, () => {
                this[timeoutProp] = null;
                try {
                    callback();
                } catch (e) {
        ...
        _readSavedMonitorSetting() {
            if (!this._settings)
                return '';

            try {
                return this._settings.get_string('monitor-setting');
            } catch (e) {
                logError('toggle.settings.monitor_setting.read_error', {
                    instanceId: this._instanceId,
                    error: `${e}`,
                });
                return '';
        ...
        _writeSavedMonitorSetting(connector) {
            if (!this._settings)
                return;

            try {
                this._settings.set_string('monitor-setting', connector);
            } catch (e) {
                logError('toggle.settings.monitor_setting.write_error', {
                    instanceId: this._instanceId,
                    connector,
                    error: `${e}`,
                });
        ...
        async _getMonitorConfig() {
            ...
            try {
                        ...
                        for (const m of lm[5]) {
                            ...
                            if (phys) {
                                const curMode = phys[1].find(mode =>
                                    typeof mode[0] === 'string' && _unpackBool(mode[6]?.['is-current'])
                                );
                ...
                if (!this._monitor && this._monitors.length > 0) {
                    ...
                    const savedConnector = this._readSavedMonitorSetting();
        ...
        _resolveModeId(connector) {
            ...
            const current = phys[1].find(m => typeof m[0] === 'string' && _unpackBool(m[6]?.['is-current']));
            if (current) return current[0];
            const preferred = phys[1].find(m => typeof m[0] === 'string' && _unpackBool(m[6]?.['is-preferred']));
        ...
        async _toggleMonitor() {
            ...
            if (isActive) {
                ...
                // Remove the selected monitor; reconstruct in ApplyMonitorsConfig format.
                // GetCurrentState returns monitors as (connector, vendor, product, serial)
                // but ApplyMonitorsConfig expects (connector, mode_id, properties).
                // We must NOT pass the raw DBus data through.
                finalLogicalMonitors = [];
                for (const lm of this._logicalMonitors) {
                    const remaining = lm[5].filter(m => m[0] !== this._monitor);
                    if (remaining.length === 0) continue;

                    const conn = remaining[0][0];
