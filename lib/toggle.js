...
import { loadMonitorConfigFromMonitorsXML, loadFullConfigFromMonitorsXML } from './xmlReader.js';
    ...
    class SecondMonitorToggle extends QuickMenuToggle {
        ...
        async _getMonitorConfig() {
            ...
            try {
                ...
                // Update original positions whenever ALL connected monitors are logically active.
                // This captures position changes made through GNOME Settings while all monitors are on.
                const allPhysicalMonitorsActive = this._monitors.length > 0 && this._monitors.every(physMon =>
                    newLogicalMonitors.some(lm => lm[5].some(m => m[0] === physMon[0][0]))
                );
                if (allPhysicalMonitorsActive) {
        ...
        async _toggleMonitor() {
            ...
            if (isSelectedMonitorActive) { 
                ...
                // Preserve existing positions when toggling off — don't recalculate.
                ...
                if (originalLmsContainingSelectedMonitor.length > 0) {
                    // Rebuild from original saved positions for ALL monitors to preserve layout.
                    // Collect connectors that should be active (currently active + the one being enabled).
                    const activeConnectors = new Set();
                    for (const lm of this._logicalMonitors) {
                        for (const m of lm[5]) activeConnectors.add(m[0]);
                    }
                    activeConnectors.add(this._monitor);

                    newLogicalMonitorsCandidate = this._originalLogicalMonitors
                        .map(origLm => {
                            const [x, y, scale, transform, isPrimary, monitors, properties] = origLm;
                            const filteredMonitors = monitors.filter(m => activeConnectors.has(m[0]));
                            if (filteredMonitors.length > 0) {
                                return JSON.parse(JSON.stringify([x, y, scale, transform, isPrimary, filteredMonitors, properties]));
                            }
                            return null;
                        })
                        .filter(lm => lm !== null);
                    ...
                    const newLmEntry = [ 
                        ...
                        initialTransform, (storedXmlConfig?.isPrimary || newLogicalMonitorsCandidate.length === 0),
                        [[connector, modeIdToUse, {}]], {} 
                    ];
                    newLogicalMonitorsCandidate.push(newLmEntry);

                    // Also restore positions and primary status for existing monitors from monitors.xml
                    // to undo any position shifts that occurred while they were the sole monitor.
                    const fullXmlConfig = loadFullConfigFromMonitorsXML(connector);
                    if (fullXmlConfig) {
                        let allPositionsFound = true;
                        for (const lm of newLogicalMonitorsCandidate) {
                            const lmConnector = lm[5][0]?.[0];
                            if (lmConnector && fullXmlConfig[lmConnector]) {
                                lm[0] = fullXmlConfig[lmConnector].x;
                                lm[1] = fullXmlConfig[lmConnector].y;
                                lm[4] = fullXmlConfig[lmConnector].isPrimary;
                            } else if (lmConnector) {
                                allPositionsFound = false;
                            }
                        }
                        if (allPositionsFound) skipRepositioning = true;
                    }
                }
            }
            ...
            if (positionedLms.length > 0) {
                ...
                if (primaryIdx === -1 || positionedLms.filter(lm => lm[4] === true).length > 1) {
                    // When restoring from originals/XML, prefer the original primary.
                    // Otherwise fall back to the current primary.
                    let desiredPrimaryConnector = null;

                    if (skipRepositioning && this._originalLogicalMonitors.length > 0) {
                        const origPrimaryLm = this._originalLogicalMonitors.find(lm => lm[4] === true);
                        if (origPrimaryLm && origPrimaryLm[5].length > 0) {
                            desiredPrimaryConnector = origPrimaryLm[5][0][0];
                        }
                    }

                    if (!desiredPrimaryConnector && this._logicalMonitors.length > 0) {
                        const currentPrimaryLm = this._logicalMonitors.find(lm => lm[4] === true);
                        if (currentPrimaryLm && currentPrimaryLm[5].length > 0) {
                            desiredPrimaryConnector = currentPrimaryLm[5][0][0];
                        }
                    }

                    if (desiredPrimaryConnector) {
                        primaryIdx = positionedLms.findIndex(lm => lm[5][0][0] === desiredPrimaryConnector);
                    }
                ...
                // Only recalculate positions when we don't have preserved original positions.
                // When positions are restored from _originalLogicalMonitors or monitors.xml,
                // skip this block to preserve the user's custom monitor layout.
