...
export const SecondMonitorToggle = GObject.registerClass(
    class SecondMonitorToggle extends QuickMenuToggle {
        ...
        _scheduleConfigRefresh(opId, errorEvent, onSuccess = null, delayMs = 1200) {
            ...
            if (this._configRefreshTimeoutId) GLib.Source.remove(this._configRefreshTimeoutId);
            this._configRefreshTimeoutId = GLib.timeout_add(GLib.PRIORITY_DEFAULT, delayMs, () => {
                this._configRefreshTimeoutId = null;
        ...
        async _toggleMonitor() {
            ...
            if (!isActive) {
                if (this._primaryDialogTimeoutId) GLib.Source.remove(this._primaryDialogTimeoutId);
                this._primaryDialogTimeoutId = GLib.timeout_add(GLib.PRIORITY_DEFAULT, 1500, () => {
                    this._primaryDialogTimeoutId = null;
                    if (!this._proxy) return GLib.SOURCE_REMOVE;
        ...
        destroy() {
            ...
            if (this._configRefreshTimeoutId) {
                GLib.Source.remove(this._configRefreshTimeoutId);
                this._configRefreshTimeoutId = null;
            }
            if (this._primaryDialogTimeoutId) {
                GLib.Source.remove(this._primaryDialogTimeoutId);
                this._primaryDialogTimeoutId = null;
            }
            if (this._makePrimaryDialog) {
                this._makePrimaryDialog.close();
